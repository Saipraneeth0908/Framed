{{ config(materialized='incremental', unique_key=['event_id', 'occurred_at'], on_schema_change='append_new_columns') }}
...
{% if is_incremental() %}
where e.occurred_at > (select coalesce(max(occurred_at), '2000-01-01'::timestamptz) from {{ this }})
                      - interval '{{ var("late_arrival_window") }}'
{% endif %}
