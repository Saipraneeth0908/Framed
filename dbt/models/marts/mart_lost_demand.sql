...
unbuildable_views as (
    ...
    join {{ source('store', 'products') }} p on p.id = e.product_id
    where e.event_name = 'product_view'
      and not exists (
          select 1
          from {{ source('store', 'variants') }} v
          join {{ source('ops', 'variant_components') }} vc on vc.variant_id = v.id
          join {{ source('ops', 'components') }} c on c.id = vc.component_id
